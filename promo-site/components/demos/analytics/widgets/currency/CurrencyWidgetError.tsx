import { RefreshCw, AlertCircle, ServerCrash, WifiOff } from 'lucide-react';

interface CurrencyWidgetErrorProps {
  error: Error;
  onRetry: () => void;
}

export function CurrencyWidgetError({ error, onRetry }: CurrencyWidgetErrorProps) {
  let errorTitle = 'Something went wrong';
  let errorDescription = 'An unexpected error occurred. Please try again.';
  let errorIcon = AlertCircle;

  if (error.message.includes('CBR_API_ERROR')) {
    errorTitle = 'Unable to fetch currency data';
    errorDescription = 'The Central Bank of Russia API is currently unavailable. Please try again later.';

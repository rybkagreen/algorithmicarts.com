import { RefreshCw, AlertCircle, Github, WifiOff } from 'lucide-react';

interface GitHubWidgetErrorProps {
  error: Error;
  onRetry: () => void;
}

export function GitHubWidgetError({ error, onRetry }: GitHubWidgetErrorProps) {
  let errorTitle = 'Something went wrong';
  let errorDescription = 'An unexpected error occurred. Please try again.';
  let errorIcon = AlertCircle;

  if (error.message.includes('GITHUB_API_ERROR')) {
    errorTitle = 'Unable to fetch GitHub data';
    errorDescription = 'The GitHub API is currently unavailable. Please try again later.';
    errorIcon = Github;
